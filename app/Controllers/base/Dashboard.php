...
class Dashboard extends Controller
{
    ...
    public function update_paket()
    {
        ...
        $data2 = [
            'invoice' => $today . $_SESSION['id'] . rand(10, 99),
            'status' => 0,
    ...
    public function do_save_tamu()
    {
        ...
        // Ambil input asli (TIDAK DIUBAH)
        $nama_tamu_input = $this->request->getPost('nama_tamu');

        // Bersihkan hanya untuk generate code
        $nama_tamu_clean = preg_replace('/[^a-zA-Z]/', '', $nama_tamu_input);

        // fallback kalau kosong
        if (empty($nama_tamu_clean)) {
            $nama_tamu_clean = 'tamu';
        }

        // === GENERATE 4 KARAKTER UNIK ===
        $unique_code = substr(str_shuffle('abcdefghijklmnopqrstuvwxyz0123456789'), 0, 4);

        // DATA ASLI (untuk display)
        $data['nama_tamu'] = ucwords($nama_tamu_input);
        ...
        // generate QR (pakai slug yang sudah unik)
        $data['qrcode'] = md5($domain . $data['nama_slug'] . $data['alamat_slug']);
